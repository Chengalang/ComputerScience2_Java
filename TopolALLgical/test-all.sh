...
if [ ! -f TopolALLgical.java ]; then
	echo ""
	echo " Error: You must place TopolALLgical.java in this directory before we"
...
do
	...
	if [ ! -f input_files/TestCase$i-solution.txt ]; then
		echo ""
		echo " Error: You must place TestCase$i-solution.txt in the input_files directory"
		echo "        before we can proceed. Aborting test script."
		echo ""
		exit
	fi
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " TopolALLgical.java | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" TopolALLgical.java | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" TopolALLgical.java | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" TopolALLgical.java -n"
		echo "     grep \"\$(printf '^\t')\" TopolALLgical.java -n"
